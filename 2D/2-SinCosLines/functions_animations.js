...
class Animate {
    constructor({
        ...
        grayscale = false,
            ...
            this.colorModulus = Math.round(this.linesAmount / (gradientReachMax * gradientCount))
            ctx.shadowOffsetX = 2
            ctx.shadowOffsetY = 0
            ctx.shadowBlur    = 0
    ...
    async lineShow() {
        ...
        return await this.#main(()=> {
            ...
            const a1 = Math.cos(angle) * this.radius * this.circleHeigth
            const a2 = Math.sin(angle) * this.radius * this.circleWidth
            const a3 = Math.cos(angle) * (this.radius * Math.cosh(this.i)) * this.circleWidth
            const a4 = Math.sin(angle) * (this.radius * Math.sin(this.i)) * this.circleWidth
    ...
    async mergingTriagles(someWhatReversed = false) {
        ...
        return await this.#main(()=> {
            ...
            const a3 = Math.tanh(angle) * (this.radius * Math.sin(this.i)) * this.circleWidth// - this.i
            const a4 = Math.cos(angle) * (this.radius * Math.cos(this.i)) * this.circleWidth //- this.i
    ...
    async linesMovement(flag = false) {
        class PointC {
            constructor(speed = 2){
                this.x = randint(Xmax)
                this.y = randint(Ymax)
                this.speed = {x:randfloat(-speed, speed), y:randfloat(-speed, speed)}
            }
            move() {
                this.x += this.speed.x
                this.y += this.speed.y
    ...
    async #main(func) {
        for (let x = 0; x < this.loopLength; x += 1) {
            ...
            clear()
            if(this.adder == false && (this.flowMid > this.horizon || this.flowMid < -this.horizon)) {
                this.adder *= -1
            }
                ...
                for (let i = 0; i < this.linesAmount; i += 1) {
                    ...
                    if(this.gradient && !this.grayscale){
                        if(i % this.colorModulus == 0) {
                            ...
                            ctx.strokeStyle = `hsla(${(overcount((i /  this.colorModulus) + this.currentGradient, this.gradientReachMax)).toFixed(0)},${this.saturation}%,50%,${this.transparency})`;
                        }
                    }
                    this.i = i
                    this.gi = gi
                    ...
                    this.b = this.startPointY  - gi * this.space - Math.cos((i + (x  * this.direction) - gi * this.cursive) * (Math.PI / this.freq )) * this.amp + randint(this.noiseY);
                    this.randomness = this.startPointY - 10 - gi * this.space - Math.sin((i - (x  * this.direction) - gi * this.cursive) * (Math.PI / this.freq  )) * this.amp + randint(this.noiseY);
