...
class Ant {
    constructor(colony) {
        ...
        this.x = colony.x
        this.y = colony.y, 
        this.color = colony.flag//COLORS.next()//
    ...
    getLineOfSight(){
        ...
        for(let _x of range(this.lineOfSight*2).map(i=>floor(this.x+i-this.lineOfSight))) {
            for(let _y of range(this.lineOfSight*2).map(i=>floor(this.y+i-this.lineOfSight))) {
                // is grid point
                if( _x % GRID.detail===0 && _y % GRID.detail===0 )  {
                    // is within circular radius
                    if ((_x-this.x)**2 + (_y-this.y)**2 <= this.lineOfSight**2) {
        ...
        return res.filter((i, idx)=> {
            ...
            if(!inRange(a, this.orientation, this.viewWideness) || inRange(a+360, this.orientation, this.viewWideness)|| inRange(a-360, this.orientation, this.viewWideness)) return i
        })
    }
    drawRaw() {
        const {x, y} = posByAngle(this.x,this. y, this.orientation, this.size*2)
        const {x:x1, y:y1} = posByAngle(this.x,this. y, this.orientation-0.7, -this.size)
        const {x:x2, y:y2} = posByAngle(this.x,this. y, this.orientation+0.7, -this.size)
    ...
    draw() {
        ...
        /** LEGS */
        const legMoveA = smoothSquareWave(((this.orientationRad*10)+this.x+this.y)/5, 0.5)/8
        const legMoveB = smoothSquareWave(((this.orientationRad*10)+this.x+this.y)/5*-1, 0.5)/8
        ...
        /** BODY */
        // back
        let {x, y} = posByAngle(this.x, this.y, this.orientationRad, this.size*-1.6);
        ellipse(x, y, this.size, this.size*0.7, this.orientationRad, 0, PI2, null, this.color);
        // middle
        ellipse(this.x, this.y, this.size, this.size*0.2, this.orientationRad, 0, PI2, null, this.color);
        // head
        ({x, y} = posByAngle(this.x, this.y, this.orientationRad, this.size*1.4));
        ellipse(x, y, this.size*0.5, this.size*0.4, this.orientationRad, 0, PI2, null, this.color);


        /** RECEPTORS */
        // 1
        let {x:rx1, y:ry1} = posByAngle(x, y, this.orientationRad-0.7, this.size*0.8);
        let {x:rx2, y:ry2} = posByAngle(x, y, this.orientationRad-0.5, this.size*1.5);
        ctx.beginPath();
        ctx.moveTo(x, y);
        ctx.lineTo(rx1, ry1);
        ctx.lineTo(rx2, ry2);
        ctx.stroke();
        // 2
        ({x:rx1, y:ry1} = posByAngle(x, y, this.orientationRad+0.7, this.size*0.8));
        ({x:rx2, y:ry2} = posByAngle(x, y, this.orientationRad+0.5, this.size*1.5));
        ctx.beginPath();
        ctx.moveTo(x, y);
        ctx.lineTo(rx1, ry1);
        ctx.lineTo(rx2, ry2);
    ...
    checkOutBounds(){
        if(this.x+this.boundPadding < Xmax && this.y+this.boundPadding < Ymax && this.x-this.boundPadding > Xmin && this.y-this.boundPadding > Ymin) {
...
class WorkerAnt extends Ant{
    ...
    evaluatePosition() {
        ...
        if(this.checkOutBounds()) {
            loginfo('- outbounds -')
            // if(this.x >= Xmax) this.targetOrientation -= mapNums()
...
class Colony {
    constructor(x, y, name, col) {
        ...
        this.ants = range(5).map(i => new WorkerAnt(this))
        this.foodStorage = 0
        this.size = 20
        this.name = name
    }
    live() {
        ...
        ctx.fillText("food: "+this.foodStorage, this.x - (this.name.length*3), this.y-20)
