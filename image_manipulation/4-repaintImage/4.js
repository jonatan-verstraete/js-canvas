            ...
            if(idx%skipSteps === 0){
                ...
                if( cData[3] != 0) {
                    res.push({
                        x: toFixed(x0+x),
                        y: toFixed(y0+y),
                        cl: `rgba(${cData[0]}, ${cData[1]}, ${cData[2]}, ${360/256*cData[0]})`
                    })
                }
            }
            idx += 1
...
async function indexImages() {
    let imagesIndexed = false, currentIndex = "";
...
const imgs = [{src:"face.png", loss: 18, size:1},{src:"hand.png", loss: 212, size: 0.5}]
const Main = async ()=> {
    let idx = 0
    await indexImages()
