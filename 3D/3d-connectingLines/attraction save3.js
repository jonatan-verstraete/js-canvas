...
class Point {
    constructor(id, speed = pointsSetup.speed) {
        ...
        this.size = 0
        this.speed = {
            x: randfloat(-speed, speed) || 0.1,
            y: randfloat(-speed, speed) || 0.1,
            z: randfloat(-speed, speed) || 0.1,
        }
        this.connections = 0
    ...
    project() {
        ...
        this.size = mapNums(this.z, 0, viewSetup.depth, pointsSetup.maxSize, pointsSetup.minSize)
        this.alpha = mapNums(this.z, 0, viewSetup.depth, 1, 0)
    }
    ...
    move() {
        ...
        //this.theta += 0.01
        this.project()
        if (pointsSetup.afterIamgeCount && (viewSetup.mode === "points" || viewSetup.mode === "shadedPoints")) this.addAfterImage(Object.copy(this))
...
class Points {
    ...
    showMovement() {
        ...
        this.points.forEach((i, idx, arr) => {
            ...
            let skipLoop = false;
            let clusterSize = 0;
            //     colorMode: [false, "setById", "setByCharge", "setByDepth"][1],
            let thisColor = "#000"
            switch (viewSetup.colorMode) {
                case "none":
                    thisColor =  hsl(0, 0, 100 - 100 * posInt(i.alpha))
                    break;
                case "setById":
                    thisColor = hsl((360/this.points.length) * i.id, 100 * posInt(i.alpha))
                    break;
                case "setByCharge":
                    thisColor = i.color
                    break;
                case "setByDepth":
                    thisColor = hsl(100 * posInt(i.alpha))
                    break;
                default:
                    thisColor =  hsl(0, 0, 100 - 100 * posInt(i.alpha))
                    return
            ...
            //const thisColor = (viewSetup.color ? hsl(100 * posInt(i.alpha)) : (viewSetup.color === null ? "black" :))
            //arr[idx].color = thisColor
            ...
            if (viewSetup.mode === "points") {
                ...
                if (pointsSetup.afterIamgeCount) {
                    i.afterImage.forEach((a, idx, arr) => point(a.projX, a.projY, (i.size / arr.length) * idx, a.color || "pink")); //a.size/((idx||1)+1)
                }
            // view: shadepoints
            } else if(viewSetup.mode === "shadedPoints") {
                shadePoint(i,  {lightStrenth:50*(2-i.alpha)})
                i.afterImage.forEach((a, idx, arr) => shadePoint(a, {lightStrenth:50*(2-a.alpha)})); //a.size/((idx||1)+1)
            ...
            // view: id
                ...
                if (dis && dis.x > 0) {
                    ...
                    // view: shapes
                        ...
                        if (viewSetup.color) ctx.fillStyle = hsl(conn.map(i => i.color).reduce((t, i) => t * i) / conn.length) //`#${ hex(round(conn.map(i=>i.color).reduce((t, i)=>t*i)/conn.length), 3).split(0, 8)[0] + '3F' }`
                        else {
...
function shadePoint(p, {lightStrenth= 100, grayscale=true, invert=true}={}) {
    try {
        ...
        const angle = posTowards({x:x, y:y}, {x:Xmid, y:Ymid}).a

        const grd = ctx.createRadialGradient(x, y, s, x+Math.cos(angle)*s, y+Math.sin(angle)*s, lightPos);
...
if(viewSetup.mode !== "shadedPoints") ctx.invert()
...
// Main function/init
async function main() {
    const m = incrementer.init(()=>{pause=true}, 10)
